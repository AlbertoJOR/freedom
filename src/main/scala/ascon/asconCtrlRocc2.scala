...
class asconCtrlRocc2 extends Module {
  ...
  val io = IO(new Bundle() {
    ...
    val block_zero = Output(Bool())
    val has_inc_plain_block = Output(Bool())
  ...
  val initialize_per = RegInit(false.B)
  ...
  val has_inc_plain_block = RegInit(false.B)
    ...
    is(s_ad) {
      ...
      when(has_associated_data) {
        ...
        when(a_len_reg >= 8.U && !io.busy_per) {
          ...
          a_len_reg := 0.U // no more blocks to absorb
