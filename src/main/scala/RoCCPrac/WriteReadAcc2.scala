...
class WriteReadAcc2(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(opcodes) {
  override lazy val module = new RWImp2(this)
}

class RWImp2(outer: WriteReadAcc2)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
 ...
 io.mem.req.bits.phys :=false.B
  ...
  switch(state) {
    ...
    is(s_mem_write_req) {
      ...
      io.mem.req.bits.tag :=  cipher_text_addr // differentiate between responses
...
class WithRWRoCC2 extends Config((site, here, up) => {
  case BuildRoCC => Seq(
    (p: Parameters) => {
      val Accel = LazyModule(new WriteReadAcc2(OpcodeSet.custom0)(p))
      Accel
    }
  )
})
