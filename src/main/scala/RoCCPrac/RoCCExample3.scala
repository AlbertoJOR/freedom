...
class AccelModuleImp3(outer: AccelModule3)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
  ...
  val MyModule = Module(new SimpleOperation3)
  MyModule.io.in_A := Cat(vec_blocks(0), vec_blocks(1)) // load
  MyModule.io.in_accum := accum_value
  ...
  switch(state) {
    ...
    is(s_wait) {
      when(io.mem.resp.valid) {
        vec_blocks(2) := MyModule.io.out_C(63, 32) // write
        vec_blocks(3) := MyModule.io.out_C(31, 0) // write
        accum_value := MyModule.io.out_C
